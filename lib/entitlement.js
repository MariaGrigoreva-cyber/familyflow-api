...
function effectiveTrialDays(nowMs = Date.now()) {
  ...
  // Порога нет — НОВАЯ ПОЛИТИКА НЕ ВКЛЮЧАЕТСЯ, даже если TRIAL_DAYS уже
  // выставлен в 14. Это защита от самой дорогой ошибки в этом механизме:
  // одной переменной, забытой в панели хостинга, достаточно, чтобы молча
  // урезать срок всем новым регистрациям — и заметить это можно через недели,
  // когда у людей начнёт заканчиваться Pro раньше обещанного.
  //
  // Смена политики требует ДВУХ осознанных действий: задать новый TRIAL_DAYS и
  // задать момент перехода. Порог нужен не только как «с какого числа»: без
  // него нигде не зафиксировано, где проходит граница когорт, и откатить
  // изменение по когорте будет уже нечем.
  if (cutoff === null) {
    if (trialDays() !== LEGACY_TRIAL_DAYS) {
      console.warn(
        `TRIAL_DAYS=${trialDays()} отличается от старой политики (${LEGACY_TRIAL_DAYS} дн.), ` +
        'но TRIAL_POLICY_CUTOFF_AT не задан — новая политика НЕ применяется, ' +
        `новые регистрации получают ${LEGACY_TRIAL_DAYS} дн. Чтобы включить новый ` +
        'срок, задайте момент перехода в TRIAL_POLICY_CUTOFF_AT.'
      );
    }
    return LEGACY_TRIAL_DAYS;
  }
