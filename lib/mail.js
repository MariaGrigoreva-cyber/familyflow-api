...
async function sendMailUni(to, subject, text, html, unsubscribeUrl) {
    ...
    const res = await fetch('https://go2.unisender.ru/ru/transactional/api/v1/email/send.json', {
      ...
      body: JSON.stringify({
        message: {
          ...
          // List-Unsubscribe — кнопка «Отписаться» в интерфейсе Gmail/Mail.ru прямо
          // рядом с отправителем, без перехода в письмо. Только "ссылочный" вариант
          // (не List-Unsubscribe-Post/one-click) — /auth/unsubscribe принимает
          // только GET, для RFC 8058 one-click нужен POST-обработчик, которого нет;
          // заявлять one-click без него было бы хуже, чем не заявлять вовсе.
...
async function sendMail(to, subject, text, html, unsubscribeUrl) {
  try {
    if (UNI_KEY) return await sendMailUni(to, subject, text, html, unsubscribeUrl);
    if (smtpTransport) {
