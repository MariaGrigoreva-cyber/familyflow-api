...
async function sendTrialEndingReminders() {
  ...
  for (const row of rows) {
    ...
    //
    // Дату КОПИРУЕМ ВНУТРИ БАЗЫ (= trial_ends_at), а не возвращаем ту, что
    // прочитали. Драйвер pg отдаёт timestamptz как JS Date, а у Date точность
    // миллисекунды против микросекунд у Postgres: записанное обратно значение
    // теряло хвост (…057616 → …057), переставало равняться trial_ends_at, и
    // отбор выше выбирал строку снова на каждом часовом прогоне. Письмо при
    // этом не дублировалось — спасала проверка в WHERE ниже, — но защита
    // держалась на тонкости, а лишняя работа повторялась вечно.
    const upd = await db.query(
      `UPDATE families SET trial_end_email_sent_for = trial_ends_at
        WHERE id = $1
          AND (trial_end_email_sent_for IS NULL OR trial_end_email_sent_for <> trial_ends_at)`,
      [row.id]
    );
