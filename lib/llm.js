...
//
// Внутри всё работает с единым форматом [{role, content}] (как у OpenAI);
// для YandexGPT content переименовывается в text уже при отправке.
const aiConfigured = () => !!process.env.AI_API_KEY;

async function askYandexGPT(messages, ctrl) {
...
async function askOpenAICompatible(messages, ctrl) {
...
// Основная точка входа: принимает готовый массив [{role, content}].
// Многоходовой диалог (system + история + текущий вопрос) идёт сюда.
async function askLLMMessages(messages) {
  if (!process.env.AI_API_KEY) throw new Error('ai not configured');
  const ctrl = new AbortController();
  const timer = setTimeout(() => ctrl.abort(), 20000);
  try {
    return process.env.AI_FOLDER_ID
      ? await askYandexGPT(messages, ctrl)
      : await askOpenAICompatible(messages, ctrl);
  } finally { clearTimeout(timer); }
}

// Прежний интерфейс «системный промпт + один вопрос» — им продолжает
// пользоваться AI-онбординг, поведение которого меняться не должно.
...
module.exports = { askLLM, askLLMMessages, aiConfigured };
