...
// виде — см. lib/crypto.js. Не блокируем запуск (ключ можно добавить позже, старые
// строки шифруются задним числом через lib/migrateEncryption.js), но громко
// предупреждаем при каждом старте, чтобы это не осталось незамеченным в проде.
if (!process.env.DATA_ENC_KEY) console.error('DATA_ENC_KEY не задан — бюджет семьи хранится в БД без шифрования');
